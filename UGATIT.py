...
class UGATIT:
    ...
    def train(self):
        ...
        for step in range(start_iter, self.iteration + 1):
            ...
            with autograd.record():
                ...
                D_ad_loss_GA = self.MSE_loss(real_GA_logit, mx.nd.ones_like(real_GA_logit)) + self.MSE_loss(fake_GA_logit, mx.nd.zeros_like(fake_GA_logit))
                D_ad_cam_loss_GA = self.MSE_loss(real_GA_cam_logit, mx.nd.ones_like(real_GA_cam_logit)) + self.MSE_loss(fake_GA_cam_logit, mx.nd.zeros_like(fake_GA_cam_logit))
                D_ad_loss_LA = self.MSE_loss(real_LA_logit, mx.nd.ones_like(real_LA_logit)) + self.MSE_loss(fake_LA_logit, mx.nd.zeros_like(fake_LA_logit))
                D_ad_cam_loss_LA = self.MSE_loss(real_LA_cam_logit, mx.nd.ones_like(real_LA_cam_logit)) + self.MSE_loss(fake_LA_cam_logit, mx.nd.zeros_like(fake_LA_cam_logit))
                D_ad_loss_GB = self.MSE_loss(real_GB_logit, mx.nd.ones_like(real_GB_logit)) + self.MSE_loss(fake_GB_logit, mx.nd.zeros_like(fake_GB_logit))
                D_ad_cam_loss_GB = self.MSE_loss(real_GB_cam_logit, mx.nd.ones_like(real_GB_cam_logit)) + self.MSE_loss(fake_GB_cam_logit, mx.nd.zeros_like(fake_GB_cam_logit))
                D_ad_loss_LB = self.MSE_loss(real_LB_logit, mx.nd.ones_like(real_LB_logit)) + self.MSE_loss(fake_LB_logit, mx.nd.zeros_like(fake_LB_logit))
                D_ad_cam_loss_LB = self.MSE_loss(real_LB_cam_logit, mx.nd.ones_like(real_LB_cam_logit)) + self.MSE_loss(fake_LB_cam_logit, mx.nd.zeros_like(fake_LB_cam_logit))
            ...
            with autograd.record():
                ...
                G_ad_loss_GA = self.MSE_loss(fake_GA_logit, mx.nd.ones_like(fake_GA_logit))
                G_ad_cam_loss_GA = self.MSE_loss(fake_GA_cam_logit, mx.nd.ones_like(fake_GA_cam_logit))
                G_ad_loss_LA = self.MSE_loss(fake_LA_logit, mx.nd.ones_like(fake_LA_logit))
                G_ad_cam_loss_LA = self.MSE_loss(fake_LA_cam_logit, mx.nd.ones_like(fake_LA_cam_logit))
                G_ad_loss_GB = self.MSE_loss(fake_GB_logit, mx.nd.ones_like(fake_GB_logit))
                G_ad_cam_loss_GB = self.MSE_loss(fake_GB_cam_logit, mx.nd.ones_like(fake_GB_cam_logit))
                G_ad_loss_LB = self.MSE_loss(fake_LB_logit, mx.nd.ones_like(fake_LB_logit))
                G_ad_cam_loss_LB = self.MSE_loss(fake_LB_cam_logit, mx.nd.ones_like(fake_LB_cam_logit))
                ...
                G_cam_loss_A = self.BCE_loss(fake_B2A_cam_logit, mx.nd.ones_like(fake_B2A_cam_logit)) + self.BCE_loss(fake_A2A_cam_logit, mx.nd.zeros_like(fake_A2A_cam_logit))
                G_cam_loss_B = self.BCE_loss(fake_A2B_cam_logit, mx.nd.ones_like(fake_A2B_cam_logit)) + self.BCE_loss(fake_B2B_cam_logit, mx.nd.zeros_like(fake_B2B_cam_logit))
    ...
    def test(self):
        ...
        model_list = os.path.join(self.result_dir, self.dataset, 'model', '*.params')
